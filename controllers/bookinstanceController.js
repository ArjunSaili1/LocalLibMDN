...
exports.bookinstance_update_get = function(req, res, next) {
    async.parallel({
        ...
        for(let i=0; i < results.books.length; i++){
            if(results.books[i] == results.bookInstance.book){
...
exports.bookinstance_update_post = [
    ...
    body('due_back').trim().isLength({min: 1}).escape().withMessage('Due back date must not be empty')
    .isISO8601().toDate().escape().withMessage("Date must be in the YYYY/MM/DD format"),
    body('status').escape(),

    (req, res, next) => {
        const errors = validationResult(req);

        var updateInstance = new BookInstance({
            book: req.body.book,
            imprint: req.body.imprint,
            due_back: req.body.due_back,
            status: req.body.status,
        ...
        if(!(errors.isEmpty())){
            ...
            BookInstance.findByIdAndUpdate(req.params.id, updateInstance, function(err, bookInstance){
                if(err){return next(err)}
                res.redirect(bookInstance.url)
            })
        }
